...
function renderTable() {
  ...
  if (!tbody) return; // ✅ Bảo vệ null
  tbody.innerHTML = "";

  students.forEach((s) => {
    if (!s.name || isNaN(s.math) || isNaN(s.physics) || isNaN(s.chemistry)) return;
...
document.getElementById("addBtn")?.addEventListener("click", () => {
  ...
  if (editId) {
    ...
    if (idx !== -1) {
      ...
      const oldStudent = students[idx] as Student;
      students[idx] = { ...oldStudent, name, math, physics, chemistry, average: avg, rank };
    }
  } else {
